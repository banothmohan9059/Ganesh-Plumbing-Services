...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const errMessage = firebaseError instanceof Error ? firebaseError.message : String(firebaseError);
      return NextResponse.json(
        { error: "Failed to save lead. Please try again later.", detail: errMessage },
        { status: 500 }
      );
    }
